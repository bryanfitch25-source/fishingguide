...
export default async function TidesPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-10 space-y-5">
      <header className="scene-panel rounded-2xl p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-3xl font-extrabold text-brand-dark mb-1">Tides</h1>
            <p className="text-muted">
              {station.name}
              {station.isDefault && " — default station"}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link
              href="/spots"
              className="rounded-lg border border-border bg-surface px-3 py-2 text-sm font-medium hover:border-brand transition"
            >
              ⭐ My Spots
            </Link>
            <Link
